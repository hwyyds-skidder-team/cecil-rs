...
use cecli_core::flags::{FileRowAttributes, MethodSemanticsAttributes, ModuleKind};
...
use crate::model::signature::{
    write_constant_blob, write_field_signature, write_method_signature, write_property_signature,
};
...
pub fn emit_metadata_with(
    ...
    if let Some(name) = asm {
        if m.kind != ModuleKind::NetModule {
            ...
            let culture = tmap.builder().insert_string(name.culture.as_deref().unwrap_or(""));
            let v = &name.version;
    ...
    for (i, res) in m.resources.iter().enumerate() {
        ...
        match res {
            ...
            Resource::AssemblyLinked { assembly, .. } => {
                ...
                impl_cell =
                    encode_coded(&coded::IMPLEMENTATION, TableIndex::AssemblyRef, pos as u32 + 1)?;
            }
        }
        ...
        let rid = tmap.builder().add_row(
            ...
            &[offset, res.attributes().bits() as u64, rname as u64, impl_cell],
        )?;
        if matches!(res, Resource::Embedded { .. }) && layout.resource_offsets.get(i).is_none() {
    ...
    for et in &m.exported_types {
        let impl_cell = match &et.implementation {
            ...
            ExportedImpl::AssemblyRef(i) => {
                encode_coded(&coded::IMPLEMENTATION, TableIndex::AssemblyRef, *i as u32 + 1)?
            }
    ...
    for (ti, td) in m.types.iter().enumerate() {
        ...
        for fid in &td.fields {
            ...
            if !fd.initial_value.is_empty() {
                while !field_data.len().is_multiple_of(FIELD_DATA_ALIGN) {
                    field_data.push(0);
                }
            ...
            if let Some(off) = fd.offset {
                sorted.field_layout.push(vec![off as u32 as u64, frid as u64]);
            }
        ...
        for mid in &td.methods {
            ...
            for ov in &md.overrides {
                ...
                let decl_cell =
                    encode_coded(&coded::METHOD_DEF_OR_REF, decl_tok.table(), decl_tok.rid())?;
                tmap.builder()
                    .add_row(TableIndex::MethodImpl, &[type_rid as u64, body_cell, decl_cell])?;
            }
        ...
        if !td.events.is_empty() {
            tmap.builder().add_row(TableIndex::EventMap, &[type_rid as u64, event_rid as u64])?;
    ...
    for (ti, td) in m.types.iter().enumerate() {
        if let Some(parent) = td.declaring_type {
            sorted.nested_class.push(vec![ti as u64 + 1, parent.0 as u64 + 1]);
        }
    }
    ...
    for ms in &pending.method_specs {
        builder
            .add_row(TableIndex::MethodSpec, &[ms.method_cell as u64, ms.instantiation as u64])?;
    }
    ...
    let entry_point_token =
        entry.map(|id| Token::new(TableIndex::MethodDef, id.0 + 1)).unwrap_or(Token::NIL);
...
fn add_parameter(
    ...
    tmap.builder()
        .add_row(TableIndex::Param, &[p.attributes.bits() as u64, sequence as u64, name as u64])?;
...
fn add_constant(
    ...
    let (tag, payload): (u8, Vec<u8>) = match value {
        ConstantValue::String(s) => {
            (ElementType::String as u8, s.encode_utf16().flat_map(|u| u.to_le_bytes()).collect())
        }
...
fn add_decl_security(
    ...
    let parent = encode_coded(group, table, rid)?;
    sorted.decl_security.push(vec![d.action as u16 as u64, parent, blob as u64]);
    Ok(())
}

/// One MethodSemantics row wiring `method` to its owning property/event.
fn add_semantic(
    _tmap: &mut TokenMap<'_>,
    ...
    sorted.method_semantics.push(vec![semantics.bits() as u64, method.0 as u64 + 1, owner]);
    Ok(())
}
...
fn table_stream_layout(root: &[u8]) -> Result<(usize, TableSet)> {
    ...
    let til = root.get(start..end).ok_or_else(|| Error::bad_image("#~ stream out of bounds"))?;
    if til.len() < 24 {
...
mod tests {
    ...
    fn sample_module() -> (Module, AssemblyNameDefinition) {
        ...
        get_x.body = Some(ResolvedBody {
            max_stack: 1,
            locals: vec![LocalVariable { index: 0, ty: i32_ty(), pinned: false }],
        ...
        m.methods.push(MethodDefinition {
            ...
            attributes: MethodAttributes::PUBLIC
                ...
                | MethodAttributes::HIDE_BY_SIG,
            signature: MethodSignature { return_type: i32_ty(), ..MethodSignature::default() },
            declaring_type: TypeId(0),
            ..MethodDefinition::default()
        });
        m.methods.push(MethodDefinition {
    ...
    fn synthetic_module_roundtrips_through_reader() {
        ...
        assert_eq!(
            strings.get(cell::<u32>(&reader, TableIndex::ManifestResource, 1, 2)).unwrap(),
            "Res.data"
        );

        // Assembly identity.
        assert_eq!(
            strings.get(cell::<u32>(&reader, TableIndex::Assembly, 1, 7)).unwrap(),
            "Sample"
        );
    ...
    fn layout_path_writes_real_values_without_patches() {
        ...
        let out = emit_metadata_with(&m, Some(&asm), Some(MethodId(0)), &layout, tmap).unwrap();
    ...
    fn deterministic_output_across_runs() {
        ...
        let same = a.root.iter().zip(b.root.iter()).all(|(x, y)| x == y);
